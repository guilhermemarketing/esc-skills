...
const CATEGORY_RULES = [
  { id: 'ads', label: '📣 Ads & PPC', icon: '📣', match: (name, desc) => /(-ads|ad-creative|advertising-creative|google-ads|meta-ads|gmp-cli)/.test(name) },
  { id: 'analytics', label: '📈 Analytics & Tracking', icon: '📈', match: (name, desc) => /(-gtm-|analytics|tracking)/.test(name) },
  { id: 'cro', label: '🎯 CRO & LPO', icon: '🎯', match: (name, desc) => /(-landing-page|-pricing-page|wireframe-landing|-icp-)/.test(name) || name === 'pricing-page' },
  { id: 'seo', label: '🔍 SEO', icon: '🔍', match: (name, desc) => name === 'seo' },
  { id: 'design', label: '🎨 Design & UI', icon: '🎨', match: (name, desc) => /^(ui-ux|frontend-|css-|responsive-|interaction-|progressive-)/.test(name) || name === 'figma-implement-design' || name.includes('design-system') || name.includes('hero-videos') || name.includes('nano-banana') },
  ...
  { id: 'content', label: '📱 Content & Social', icon: '📱', match: (name, desc) => /(threads-|viral-content|content-creation)/.test(name) },
  { id: 'cloud', label: '☁️ Cloud', icon: '☁️', match: (name, desc) => name.startsWith('aws-') },
];

// All guimkt-* skills are ALWAYS featured (auto-detected by prefix).
// Non-guimkt skills that should also be featured go in this list.
const FEATURED_SKILLS_EXTRA = [
  'skill-creator', 'ui-ux-pro-max'
];

// Extra categories for skills that belong to multiple categories
const EXTRA_CATEGORIES = {
  'guimkt-classic-advertising-creative': [
...
for (const dirName of skillDirs) {
  ...
  const skillName_ = name || dirName;
  const isFeatured = skillName_.startsWith('guimkt-') || FEATURED_SKILLS_EXTRA.includes(skillName_);
  ...
  skills.push({
    ...
    preview,
    category,
    extraCategories: extraCats.filter(ec => ec.id !== category.id).length > 0 ? extraCats.filter(ec => ec.id !== category.id) : undefined,
    fileCount,
    isFeatured,
